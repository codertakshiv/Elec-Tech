    ...
    <section className="space-y-4 animate-tabIn">
      ...
      <div className="glass-card">
        ...
        <div className="component-stage mt-4">
          ...
          <div className="smd-chip capacitor">
            <span className="smd-chip-mark">{code || '---'}</span>
          </div>
          <span className="smd-pad" />
        </div>
        <p className="mt-3 text-center text-xs text-slate-300">
          {result ? `${formatCapValue(result.uF)} uF (${formatCapValue(result.pF)} pF)` : 'Enter capacitor code to sync printed value and decoded result.'}
        </p>
      </div>
